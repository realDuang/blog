...
interface BilibiliOutroPortraitProps {
  visual: BilibiliOutroVisual;
  theme: Theme;
}

// Bilibili-style action icons (same as landscape version)
...
const CoinIcon: React.FC<{ size: number; color: string }> = ({ size, color }) => (
  <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
    ...
    <text x="18" y="23" textAnchor="middle" fontSize="15" fontWeight="bold" fontFamily="serif" fill={color}>
      币
    </text>
  </svg>
);
...
const FollowIcon: React.FC<{ size: number; color: string }> = ({ size, color }) => (
  <svg viewBox="0 0 36 36" width={size} height={size} fill="none">
    ...
    <path d="M7 27c0-4.4 3.6-8 8-8s8 3.6 8 8" stroke={color} strokeWidth="2.5" fill="none" strokeLinecap="round" />
...
const ACTION_CONFIG: Record<string, { Icon: React.FC<{ size: number; color: string }>; color: string }> = {
...
function getActionConfig(element: string) {
  ...
  if (element.includes("赞") || element.includes("like")) return ACTION_CONFIG["点赞"];
  if (element.includes("币") || element.includes("coin")) return ACTION_CONFIG["投币"];
  if (element.includes("藏") || element.includes("fav")) return ACTION_CONFIG["收藏"];
  if (element.includes("注") || element.includes("sub")) return ACTION_CONFIG["关注"];
  return { Icon: StarIcon, color: "#3B82F6" };
}

export const BilibiliOutroPortrait: React.FC<BilibiliOutroPortraitProps> = ({ visual, theme }) => {
    ...
    >
      ...
      >
        {visual.elements.map((element, i) => {
          ...
          const glowOpacity = glowPhase > 0 ? 0.3 + 0.15 * Math.sin(glowPhase * 0.15) : 0;
            ...
            >
              <div
                style={{
                  ...
                  justifyContent: "center",
                  alignItems: "center",
                  boxShadow: `0 0 ${20 + glowPhase * 0.3}px ${color}${Math.round(glowOpacity * 255).toString(16).padStart(2, "0")}`,
                }}
              >
